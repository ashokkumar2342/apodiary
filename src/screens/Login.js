...
import { View,Text,Image,TextInput,TouchableOpacity,StyleSheet,Alert,Picker,AsyncStorage,Platform } from 'react-native';
...
export class Login extends Component {
  ...
  /* Return 0 if not  1 if savailable*/
  Check_network_connection = async ()=>{  
   var lo_net_connection = 0;  
   ...
   await this.setState({ 
     s_net_connection: lo_net_connection
   })
   console.log(this.state.s_net_connection)
 
  }  

  Create_Local_DB = async ()=>{
    try{
      ...
      this.setState({download_status:'Configuration Started'});
      ...
      let userdetails_check = await connection.executeQuery("SELECT * FROM app_users WHERE mobileno = '"+mobile_no+"' and `app_installed` = 1 LIMIT 1");
      ...
      alert("there is some problem plz configure again");
      return 0;
    } 
  
    if (userdetails_check.length == 0) {
      alert("there is some problem plz configure again");
      return 0;
    }else{
      return 1;
  ...
  } //End function Create_Local_DB   

  configureData = async ()=>{  
    var configure_status = 0;
    var check_network = 0;
    try {
        this.Check_network_connection();
        check_network =this.state.s_net_connection;
      if (check_network == 1){
        const {mobile_no}=this.state  
        const connection = await MySqlConnection.createConnection(config);
        let userdetails = await connection.executeQuery("SELECT * FROM app_users WHERE mobileno = '"+mobile_no+"' LIMIT 1");
        ...
        if (userdetails.length == 0) {
          ...
          configure_status = this.Create_Local_DB(); 

          if(configure_status == 1){
