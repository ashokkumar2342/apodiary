 ...
 export class Dashboard extends Component {
  ...
  constructor(props) {
   ...
   uninstallvapp = async ()=>{
     try{
       //const connection = await MySqlConnection.createConnection(config);
       //let update_polled = await connection.executeUpdate("update `app_users` set `app_installed` = 0 where " + this.state.s_status_activity[l_counter] + ", `updated_by` = " + this.state.s_user_id + ", `updated_at` = now() where `booth_id` = " + this.state.s_booth_id + " and `districts_id` = " + this.state.s_d_id + " and `blocks_id` = " + this.state.s_b_id + " and `village_id` = " + this.state.s_v_id + " LIMIT 1");      
